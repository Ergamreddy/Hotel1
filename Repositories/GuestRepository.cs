...
public class GuestRepository : BaseRepository, IGuestRepository
{
    ...
    {
        var query = $@"INSERT INTO guest (guest_id,first_name,last_name,mobile,address,gender) VALUES (@GuestId, @FirstName, @LastName, @Mobile, @Address, @Gender) RETURNING * ";
    ...
    {
        var query = $@"UPDATE guest SET  last_name=@LastName,mobile=@Mobile,address=@Address WHERE guest_id=@GuestId ";
        using (var connection = NewConnection)
        {
